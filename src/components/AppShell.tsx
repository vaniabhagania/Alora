...
export function AppShell({ currentPage, onNavigate, children }: AppShellProps) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* Desktop sidebar */}
      <aside
        className={`hidden md:flex flex-col shrink-0 border-r border-ink/8 bg-[var(--bg-secondary)]/60 backdrop-blur-xl transition-all duration-300 ${
              ...
              <button
                ...
                className={`group mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all ${
                  active
                    ...
                    : 'text-[var(--text-secondary)] hover:bg-ink/5 hover:text-[var(--text-primary)]'
                }`}
                title={item.label}
              >
                <Icon
        ...
        <div className="border-t border-ink/8 px-3 py-3">
          {!collapsed && activeWorld && (
            <button
              ...
              className="mb-2 flex w-full items-center gap-2.5 rounded-xl px-3 py-2 transition-colors hover:bg-ink/5"
            >
              <div
          ...
          <button
            ...
            className="hidden w-full items-center justify-center rounded-xl px-3 py-2 text-xs text-[var(--text-secondary)] hover:bg-ink/5 hover:text-[var(--text-primary)]"
          >
            {collapsed ? '→' : '← Collapse'}
          </button>
          <button
