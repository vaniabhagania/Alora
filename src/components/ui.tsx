...
export function EmptyState({ icon, title, message, action }: EmptyStateProps) {
  return (
    <div className="empty-state">
      <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-ink/8 bg-ink/[0.02]">
        {icon}
      </div>
...
export function LoadingSpinner({ size = 24, label }: LoadingSpinnerProps) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-8">
      <div
        className="animate-spin-slow rounded-full border-2 border-ink/10"
        style={{
          width: size,
          height: size,
...
export function ProgressBar({ value, max = 100, className = '' }: ProgressBarProps) {
  ...
  return (
    <div className={`h-2 w-full overflow-hidden rounded-full bg-ink/5 ${className}`}>
      <div
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-[160] mt-1 min-w-[180px] overflow-hidden rounded-xl border border-ink/10 bg-[var(--bg-secondary)] shadow-2xl animate-scale-in">
            {items.map((item, i) => (
              <button
                key={i}
                onClick={(e) => { e.stopPropagation(); setOpen(false); item.onClick(); }}
                className={`flex w-full items-center gap-2.5 px-4 py-2.5 text-sm transition-colors hover:bg-ink/5 ${
